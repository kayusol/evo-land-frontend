...
export default function Mining() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const [ls, as, ds] = await Promise.all([
        Promise.all(lids.map(async id => { const [x, y] = await lc.read.decodeId([BigInt(id)]); const a = await lc.read.resourceAttr([BigInt(id)]); return { tokenId: id, x: Number(x), y: Number(y), rates: decodeRates(a) } })),
  ...
  const loadSlots = useCallback(async (landId) => {
    ...
    try {
      ...
      setPending(rw.map(r => parseFloat(formatEther(r))))
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (dId !== 0n) { const dc = getContract({ address: CONTRACTS.drill, abi: DRILL_ABI, client: wal }); h = await dc.write.setApprovalForAll([CONTRACTS.mining, true]); await pub.waitForTransactionReceipt({ hash: h }) }
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      toast.ok('领取成功', '资源已到账'); loadSlots(active.tokenId)
    } catch (e) { toast.err('操作失败', e.message?.slice(0, 80)) }
    finally { setTxKey('') }
  }

  if (!address) return <div style={{ padding:24 }}><div className="nc"><div className="nc-icon">⛏</div><h3>请先连接钱包</h3></div></div>
  if (!dep) return <div style={{ padding:24 }}><div className="lp-notice"><span style={{color:'#fbbf24'}}>⚠</span> 合约未部署</div></div>
  ...
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', overflow:'hidden' }}>
      ...
      <div style={{ flex:1, display:'grid', gridTemplateColumns:'200px 1fr', overflow:'hidden' }}>
        {/* 地块列表 */}
        <div style={{ borderRight:'1px solid rgba(255,255,255,0.07)', overflowY:'auto', padding:12, display:'flex', flexDirection:'column', gap:6 }}>
          <div style={{ fontSize:12, fontWeight:600, color:'#64748b', marginBottom:4 }}>选择地块</div>
          {loading ? <div className="skeleton" style={{ height:60 }} /> :
           lands.length === 0 ? <p style={{ fontSize:12, color:'#475569' }}>暂无地块</p> :
           lands.map(l => {
             const mainType = l.rates.indexOf(Math.max(...l.rates))
             return (
               <div key={l.tokenId} onClick={() => setActive(l)} style={{
                 background: active?.tokenId === l.tokenId ? 'rgba(74,222,128,0.08)' : 'rgba(255,255,255,0.03)',
                 border: `1px solid ${active?.tokenId === l.tokenId ? 'rgba(74,222,128,0.3)' : 'rgba(255,255,255,0.06)'}`,
                 borderRadius:8, padding:'8px 10px', cursor:'pointer', transition:'all 0.15s',
               }}>
                 <div style={{ fontFamily:'monospace', fontSize:11, color: active?.tokenId===l.tokenId ? '#4ade80' : '#94a3b8' }}>#{String(l.tokenId).padStart(5,'0')}</div>
                 <div style={{ fontSize:10, color:'#475569', marginTop:2 }}>({l.x},{l.y})</div>
                 <div style={{ display:'flex', gap:2, marginTop:4, alignItems:'flex-end', height:14 }}>
                   {l.rates.map((r,i)=><div key={i} style={{ width:8, background:RES_COLORS[i], opacity:0.4+r/255*0.6, height:Math.max(2,r/20), borderRadius:1 }} />)}
                 </div>
               </div>
             )
           })}
        ...
        <div style={{ overflowY:'auto', padding:20 }}>
          {!active ? (
            <div className="empty"><div className="empty-icon">⛰</div><h4>请选择一块地</h4><p>选择左侧地块，管理使徒和钻头</p></div>
          ) : (
            <>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:16, flexWrap:'wrap', gap:10 }}>
                <div>
                  <h2 style={{ fontSize:16, fontWeight:700, color:'#f1f5f9' }}>地块 #{String(active.tokenId).padStart(5,'0')}</h2>
                  <p style={{ fontSize:12, color:'#64748b' }}>坐标 ({active.x},{active.y}) · {slots.length}/5 挖矿位</p>
                </div>
                <div style={{ display:'flex', gap:8 }}>
                  <button className="btn btn-sm" onClick={handleClaim} disabled={!!txKey || pending.every(v => v === 0)}>{txKey==='claim'?<span className="spin-anim">◌</span>:null} 领取资源</button>
                  <button className="btn btn-primary btn-sm" onClick={() => setModal({show:true,apo:'',drill:'0'})} disabled={slots.length>=5}>+ 派遣使徒</button>
                </div>
              </div>

              {/* 待领取 */}
              <div style={{ background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:8, padding:12, marginBottom:16 }}>
                <div style={{ fontSize:11, color:'#475569', marginBottom:8, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em' }}>待领取资源</div>
              ...
              {/* 使徒列表 */}
              <div style={{ fontSize:11, color:'#64748b', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:8 }}>挖矿中的使徒</div>
              {slots.length === 0 ? (
                <p style={{ fontSize:12, color:'#475569' }}>暂无使徒在此挖矿，点击「派遣使徒」开始</p>
              ) : slots.map((s, i) => (
                <div key={i} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 14px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:8, marginBottom:8 }}>
                  <span style={{ fontSize:22 }}>🧙</span>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13, fontWeight:600, color:'#f1f5f9' }}>使徒 #{s.apo}</div>
                    {s.drill > 0 && <div style={{ fontSize:11, color:'#94a3b8' }}>⛏ 配备钻头 #{s.drill}</div>}
                    <div style={{ fontSize:10, color:'#475569' }}>开始时间: {new Date(s.t*1000).toLocaleString('zh-CN')}</div>
                  </div>
                  <button className="btn btn-danger btn-sm" onClick={() => handleStop(s.apo)} disabled={!!txKey}>{txKey==='s'+s.apo?<span className="spin-anim">◌</span>:'撤回'}</button>
                </div>
              ))}
            </>
          )}
      ...
      {modal.show && (
        <div className="modal-bg" onClick={() => setModal(m => ({...m,show:false}))}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-head"><h3>派遣使徒</h3><button className="modal-close" onClick={() => setModal(m=>({...m,show:false}))}>×</button></div>
            <div className="form-group">
              <label>选择使徒</label>
              <select value={modal.apo} onChange={e => setModal(m => ({...m,apo:e.target.value}))}>
                <option value="">-- 请选择使徒 --</option>
                {apostles.map(a => <option key={a.tokenId} value={a.tokenId}>#{a.tokenId} · 力量 {a.strength} · {['黄金','木材','水源','火焰','土地'][a.element]}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label>选择钻头（可选）</label>
              <select value={modal.drill} onChange={e => setModal(m => ({...m,drill:e.target.value}))}>
                ...
                {drills.map(d => <option key={d.tokenId} value={d.tokenId}>#{d.tokenId} · {d.tier}星 · {['黄金','木材','水源','火焰','土地'][d.affinity]}亲和 +{d.tier*20}%</option>)}
              </select>
            </div>
            <div className="modal-foot">
              <button className="btn" onClick={() => setModal(m=>({...m,show:false}))}>取消</button>
              <button className="btn btn-primary" onClick={handleStart} disabled={!modal.apo||txKey==='start'}>{txKey==='start'?<span className="spin-anim">◌</span>:null} 派遣</button>
            </div>
          </div>
        </div>
      )}
